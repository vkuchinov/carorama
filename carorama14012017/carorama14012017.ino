...
/*
  ...
  //_название_вирутального_объекта_.write("AT+NAMECARORAMA");
  //в текущем коде _название_вирутального_объекта__название_вирутального_объекта_ = BT_HC06
...
//УНИКАЛЬНЫЙ КОД ПРИЛОЖЕНИЯ, СМОТРЕТЬ В BLYNK
char auth[] = "3507f788cebb43ac85f87ba3f9fe19bd";
...
int  MOTOR_RESOLUTION = 200;           // физическое количество шагов на круг
int  GEARBOX = 5;                      // шестерная передача
...
int  STEPS = 8;            // количество снимков на круг [управляется с телефона]
int  STABILIZATION = 3000; // задержка для стабилизации [управляется с телефона]
int  EXPOSURE = 10000;     // выдержка [управляется с телефона]
...
void StepperMotor::shift(int dir_) {
  digitalWrite(ports[2], LOW);
  long steps = floor(NUM_OF_IMPULSES_PER_TURN / STEPS);
...
void setup() {
  ...
  Serial.println("CARORAMA IS READY...");
  timer.setInterval(250L, updateParameters);

  Blynk.virtualWrite(V20, "УСТРОЙСТВО");
  Blynk.virtualWrite(V21, "ГОТОВО");
...
void scenarioA() {
  ...
  Blynk.virtualWrite(V20, "СЪЕМКА");
  Serial.println("Starting main sequince");
  ...
  for (int i = 0; i < STEPS; i++) {
    ...
    motorA->shift(CW);
    delay(STABILIZATION);
    Blynk.virtualWrite(V21, "ВЫПОЛНЕНО " + String(i) + " ИЗ " + String(STEPS));
...
void singleShot() {
  ...
  Blynk.virtualWrite(V20, "ОДИНОЧНЫЙ КАДР");
  Blynk.virtualWrite(V21, " ");
...
String parseData(String data_) {
  ...
  if (data_.indexOf('_') < 0) {
    ...
    if (data_.indexOf("CW") >= 0) {
      ...
      STABILIZATION = value.toFloat() * 1000;
      listener->sendIdle();
    }
    else if (data_.indexOf("EXP") >= 0) {
      listener->set(false);
      Serial.println(value);
      EXPOSURE = value.toFloat() * 1000;
      listener->sendIdle();
    }
...
void updateParameters(){
  ...
  Blynk.virtualWrite(V10, STEPS);
  delay(10);
  Blynk.virtualWrite(V11, STABILIZATION / 1000);
  delay(10);
  Blynk.virtualWrite(V12, EXPOSURE / 1000);
  delay(10);
...
BLYNK_WRITE(V0) { if(param.asInt() == 1 ) { scenarioA(); } }
BLYNK_WRITE(V1) { if(param.asInt() == 1 ) { singleShot();  } }
