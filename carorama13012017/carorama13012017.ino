...
Listener::Listener(int port_) { this->port = port_; this->last = millis(); }
void Listener::set(bool state_) { this->state = state_; if(this->state) { digitalWrite(this->port, HIGH); } else { digitalWrite(this->port, LOW); } }
void Listener::sendIdle() { while(!handshaked) { BT_HC06.print("![IDLE]"); this->getOK(); } this->set(true); handshaked = false; }
void Listener::getOK() { 
  ...
  if (metadata.length() > 0 && metadata.indexOf("OK") >=0) { String m = "[HANDSHAKE] in " + String((millis()-this->last)) + " millis"; this->handshaked = true; Serial.println(m); delay(50); }
...
void Listener::getStop() { 
  ...
  if (metadata.length() > 0 && metadata.indexOf("STOP") >=0) { this->handshaked = true; }
...
void setup() {
  ...
  if(BT_HC06.available()) { listener->sendIdle(); }
...
String parseData(String data_){
   ...
   if(data_.indexOf("STR") >= 0) {  listener->set(false);  scenarioA(); listener->sendIdle(); }
   else if(data_.indexOf("SNG") >= 0) { listener->set(false);  singleShot();  listener->sendIdle(); }
   else if(data_.indexOf("FLSN") >= 0) { listener->set(false); Serial.println("flash on"); listener->sendIdle(); }
   else if(data_.indexOf("FLSF") >= 0) { listener->set(false); Serial.println("flash off"); listener->sendIdle(); }
   ...
   if(data_.indexOf("CW") >= 0) { listener->set(false); int val = round(NUM_OF_IMPULSES_PER_TURN / 360.0 * value.toInt()); motorA->moveNumOfSteps(val, CCW); listener->sendIdle(); } 
   else if(data_.indexOf("CL") >= 0 ) { listener->set(false); int val = round(NUM_OF_IMPULSES_PER_TURN / 360.0 * value.toInt()); motorA->moveNumOfSteps(val, CW); listener->sendIdle(); }
   else if(data_.indexOf("ST") >= 0) { listener->set(false); INTERVAL_A = value.toInt() * 1000;  listener->sendIdle();  }
   else if(data_.indexOf("EXP") >= 0) { listener->set(false); INTERVAL_B = value.toInt() * 1000; listener->sendIdle();}
   else if(data_.indexOf("SLZ") >= 0) { listener->set(false);  SHIFTS = value.toInt();  listener->sendIdle(); }
