...
int SHIFTS = 8;
int GEARBOX = 5;
int number_of_impulses_per_turn = 32000; //dynamic

int INTERVAL_A = 750; //settle
int INTERVAL_B = 500; //exposure
...
void StepperMotor::inits() { 
    ...
    number_of_impulses_per_turn = fullTurn * resolution * GEARBOX;
    Serial.print("There are ");
    Serial.print(number_of_impulses_per_turn);
...
void StepperMotor::shift(int dir_){ digitalWrite(ports[2], LOW); long steps = floor(number_of_impulses_per_turn / SHIFTS); for(int s = 0; s < steps; s++){ if(dir_ == 1) { stepCW(); } else { stepCCW(); }} digitalWrite(this->ports[1], LOW); digitalWrite(ports[2], HIGH); }
